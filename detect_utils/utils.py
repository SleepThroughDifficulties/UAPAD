...
class Huggingface_dataset(Dataset):
    def __init__(
        ...
        self.subset = subset if subset != 'none' else None
        ...
        print(f'INFO: Load dataset {self.name}-{self.subset}\n')
...
class local_dataset(Dataset):
    def __init__(
        ...
        print(f'INFO: Load local dataset {self.dataset_name} from: {self.dataset_path}\n')

        self.dataset = datasets.load_dataset(self.type, data_files=self.dataset_path, delimiter='\t')[split]
        
        self.input_columns = task_to_keys[self.dataset_name]
