from sklearn.metrics import precision_recall_curve, average_precision_score, auc, roc_curve, accuracy_score
...
class oodMetrics():
    ...
    def compute_all_metrics(self,y_gt,y_pred,choose_thresholds=True, fixed_threshold=None):
        ...
        # # acc
        # tmp_thre_list = [0 + 0.1*i for i in range(0, 101)]
        # self.metrics["ACCs"] = []
        # for tmp_thre in tmp_thre_list:
        #     self.metrics["ACCs"].append(
        #         sum(np.where(y_pred > tmp_thre, 1, 0) == y_gt) / len(y_gt)
        #     )
        
        # self.metrics["ACC"] = max(self.metrics["ACCs"])
        ...
        # self.metrics["max_tpr"] = max(self.metrics["tpr"])
    ...
    def pretty_print_metrics(self):
        logger.info("-----OOD.METRICS.SUMMARY.BEGIN-----")
        ...
        for key in keyList:
            ...
            logger.info("{}: {}".format(key, self.metrics[key]))
        logger.info("-----OOD.METRICS.SUMMARY.END-----")



if __name__ == "__main__":

    df = pandas.read_csv('./detectResult_final/bert/ag_news/none/textfooler/loss/advsteps_6/advlr_0.01/threshold_0.0/pred_easy.csv')
    preds = np.array(df)[:,0]
    gt = np.array(df)[:,1]

    met = oodMetrics()
    met.compute_all_metrics(gt, preds)

    for k,v in met.metrics.items():
        if isinstance(v, float):
            print(f'{k}: {v}')
    print(met.metrics['tpr'])
    print(len(met.metrics['tpr']))